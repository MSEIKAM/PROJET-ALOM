...
public class UserResource {
	 ...
	 * String token = userService.verifyCredentials(user.getNickname(),
	 * user.getPwd());
	 * if (token != null) {
	 ...
	 * } else {
	 * return
	 * Response.status(Response.Status.UNAUTHORIZED).entity("Invalid credentials").
	 * build();
	 * }
	 * }
	 */
		...
		try {
			...
			if (token == null || token.isEmpty()) {
				...
				if (user != null) {
					...
					return Response.status(Response.Status.NOT_FOUND).entity("User not found or not connected.")
							.build();
				}
			}
		} catch (Exception e) {
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response getUsername(String jsonPayload) {
		try {
			JSONObject json = new JSONObject(jsonPayload);
			String token = json.getString("token");
